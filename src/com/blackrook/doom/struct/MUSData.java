 ...
 */
public class MUSData implements DoomObject
{
		...
		try {
			...
			for (Event e : eventList)
			{
				...
				switch (e.getType())
				{
					...
					{
						EventNotePlay c = (EventNotePlay)e;
					...
					{
						EventNoteRelease c = (EventNoteRelease)e;
						if (c.getChannel() == CHANNEL_DRUM)	// drum channel
						{
							byte n = c.getNote();
							if (n >= 35 && n <= 81)
					...
					{
						EventControllerChange c = (EventControllerChange)e;
						if (c.getChannel() != CHANNEL_DRUM && c.getController() == EventControllerChange.CONTROLLER_INSTRUMENT)
						{
							int inst = c.getValue();
							if (!instruments.contains(inst))
								instruments.add(inst);
		...
		while (!foundEnd)
		{
			...
			switch ((eventDesc & 0x70) >>> 4)
			{
				case Event.TYPE_RELEASE:
				{
					...
					eventList.add(new EventNoteRelease(channel, b, last, tics));
				}
					break;
				...
				case Event.TYPE_PLAY:
				{
					...
					byte volume = EventNotePlay.VOLUME_NO_CHANGE;
					if (Common.bitIsSet(b, 0x80))
						volume = sr.readByte();
					int tics = 0;
					if (last)
						tics = sr.readVariableLengthInt();
					eventList.add(new EventNotePlay(channel, note, volume, last, tics));
				}
					break;
				...
				case Event.TYPE_PITCH:
				{
					...
					if (last)
						tics = sr.readVariableLengthInt();
					eventList.add(new EventPitch(channel, b, last, tics));
				}
					break;
				...
				case Event.TYPE_SYSTEM:
				{
					...
					if (last)
						tics = sr.readVariableLengthInt();
					eventList.add(new EventSystem(channel, b, last, tics));
				}
					break;
				...
				case Event.TYPE_CHANGE_CONTROLLER:
				{
					...
					if (last)
						tics = sr.readVariableLengthInt();
					eventList.add(new EventControllerChange(channel, b, b2, last, tics));
				}
					break;
				...
				case Event.TYPE_SCORE_END:
				{
					...
					if (last)
						tics = sr.readVariableLengthInt();
					eventList.add(new EventScoreEnd(channel, last, tics));
					foundEnd = true;
				}
					break;
			}
	 ...
	 * Note release event.
	 */
	public static class EventNoteRelease extends Event
	{
		 ...
		 */
		public EventNoteRelease(byte channel, byte note)
		{
		 ...
		 * @param restTics	The amount of tics before the next event gets processed.
		 */
		public EventNoteRelease(byte channel, byte note, boolean last, int restTics)
		{
	 ...
	 * Note play event.
	 */
	public static class EventNotePlay extends Event
	{
		 ...
		 */
		public EventNotePlay(byte channel, byte note, byte volume)
		{
		 ...
		 * @param restTics	The amount of tics before the next event gets processed.
		 */
		public EventNotePlay(byte channel, byte note, byte volume, boolean last, int restTics)
		{
	 ...
	 * Pitch wheel event.
	 */
	public static class EventPitch extends Event
	{
		 ...
		 */
		public EventPitch(byte channel, short pitch)
		{
		 ...
		 * @param restTics	The amount of tics before the next event gets processed.
		 */
		public EventPitch(byte channel, short pitch, boolean last, int restTics)
		{
	 ...
	 * System event.
	 */
	public static class EventSystem extends Event
	{
		 ...
		 */
		public EventSystem(byte channel, byte sysType)
		{
		 ...
		 * @param restTics	The amount of tics before the next event gets processed.
		 */
		public EventSystem(byte channel, byte sysType, boolean last, int restTics)
		{
	 ...
	 * Controller Change event.
	 */
	public static class EventControllerChange extends Event
	{
		 ...
		 */
		public EventControllerChange(byte channel, byte controllerNumber, byte controllerValue)
		{
		 ...
		 * @param restTics			The amount of tics before the next event gets processed.
		 */
		public EventControllerChange(byte channel, byte controllerNumber, byte controllerValue, boolean last, int restTics)
		{
	 ...
	 * Score ending event.
	 */
	public static class EventScoreEnd extends Event
	{
		/**
		 * Creates a "score ending" event.
		 * @param channel	Event channel.
		 */
		public EventScoreEnd(byte channel)
		{
		 ...
		 * @param restTics	The amount of tics before the next event gets processed.
		 */
		public EventScoreEnd(byte channel, boolean last, int restTics)
		{
