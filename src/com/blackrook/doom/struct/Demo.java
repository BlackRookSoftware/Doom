 ...
 * all things equal.
 * <p>
 * The method {@link #readDoomBytes(InputStream)} will read until it detects the end of the DEMO
 * information.
 * @since 2.6.0
 * @author Matthew Tropiano
 */
public class Demo implements DoomObject
{
	...
	{
		checkByteUnsigned("Version", version);
		checkRange("Skill", 0, 4, skill);
		checkByteUnsigned("Episode", episode);
		checkByteUnsigned("Map", map);
		checkByteUnsigned("Viewpoint", viewpoint);
		checkByteUnsigned("Players", players);
	}
	...
	public byte[] getDoomBytes() throws DataExportException
	{
		...
		if (!(VERSION0 || VERSION1 || VERSION2))
			throw new DataExportException("Bad Demo Version");
